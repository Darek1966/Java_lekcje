public class TypyProste {

        // liczby stałoprzecinkowe (można przypisać wartości jn.)
        byte firstNumber = 127;         // 1 bajt od - 128 do 127
        short secondNumber = 32689;     // 2 bajty od - 32768 do 32767
        int thirdNumber = 32768999;     // 4 bajty od - 2 147 483 648 do 2 147 483 647
        long fourthNumber = 2148000L;   // 8 bajtów od - 2^63 do (2^63) -1 (L-long)

        // liczby zmiennoprzecinkowe
        float fifthNumber = 4.99934F;   // 4 bajty - max ok. 6 - 7 liczb po przecinku (F-float)
        double sixthNumber = 3.999999999999999D;    // 8 bajtów - max do 15 cyfr po przecinku (D-double)

        // wartość logiczna
        boolean prawda = true;
        boolean falsz = false;

        // pojedyczy znak
        char letter = 'A';

}
